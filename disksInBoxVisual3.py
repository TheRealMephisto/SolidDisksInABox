...
for k in range(0, 1000000):
    ...
    if k % 100 == 0:
        ...
        if firstTime:
            df.to_csv('e3.csv', index=False)
            firstTime = False
        else:
            df.to_csv('e3.csv', mode='a', index=False, header=False)
        dataToSave = []

    [state, d, e] = util.updateDisksInBox(state, d, e, L, N, r, a, 10)
